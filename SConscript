...
if   env['toolchain']=='mspgcc':
    ...
    if env['board'] not in ['OpenMote-CC2538','iot-lab_M3','VESNA','VESNA_AT86RF212']:
        raise SystemError('toolchain {0} can not be used for board {1}'.format(env['toolchain'],env['board']))
    ...
    if   env['board']=='OpenMote-CC2538':
    ...
    elif env['board']=='VESNA_AT86RF212':
          # compiler (C)
        env.Replace(CC           = 'arm-none-eabi-gcc')
        if os.name=='nt':
            env.Append(CCFLAGS   = '-DHSE_VALUE=((uint32_t)16000000)')        
        ...
        env.Append(LINKFLAGS     = '-nostartfiles')
        env.Append(LINKFLAGS     = '-Tbsp/boards/VESNA/STM32_512K_64K_FLASH.ld')
        env.Append(LINKFLAGS     = os.path.join('build','VESNA_AT86RF212_armgcc','bsp','boards','VESNA_AT86RF212','startup.o'))
        env.Append(LINKFLAGS     = os.path.join('build','VESNA_AT86RF212_armgcc','bsp','boards','VESNA_AT86RF212','configure','stm32f10x_it.o'))
        # assembler
        env.Replace(AS           = 'arm-none-eabi-as')
        env.Append(ASFLAGS       = '-ggdb -g3 -mcpu=cortex-m3 -mlittle-endian')  
        # object manipulation
